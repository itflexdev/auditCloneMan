...
class Company_allocatecoc_Model extends CC_Model
{
	...
	public function getstockList($type, $requestdata=[]){ 		
		...
		$this->db->select('sm.*,ud.name as name,ud.surname as surname,up.registration_no as registration_no,pa.invoiceno as invoiceno,pd.company as company ');
		...
		if(isset($requestdata['roletype']) && $requestdata['roletype']=='6'){
			...
			if (!isset($requestdata['querytype']) && $requestdata['querytype'] =='companystocks') {
	...
	public function action($data)
	{
		...
		$companycoc = $this->getstockList('all', ['roletype' => '6', 'user_id' => $companyid, 'querytype' => 'companystocks']);
		// echo "<pre>";print_r($this->db->last_query());die;
		if(isset($request)){
			$range = $data['rangebalace_coc'];

			for($i=0; $i < $range; $i++){
				$request['coc_id'] =	$companycoc[$i]['id'];
				$users = $this->db->insert('plumberallocate_company', $request);
				$plumberallocate = $this->db->insert_id();
				$idarray['plumberallocateid'] = $plumberallocate;
			}
			...
			for($i=0; $i < $range; $i++){
				...
				$request2['allocatedby'] 		= $companyid;
				$updateid 						= $companycoc[$i]['id'];
				$users1 = $this->db->update('stock_management', $request2, ['id' => $updateid]);
				$idarray['stockid'][] = $updateid;
			...
			$idarray['cocupdateid'] = $cocupdateid;
			...
			$users11 				= $this->db->update('coc_count', $request11, ['user_id' => $companyid]);
			$idarray['companyid'] 	= $companyid;
