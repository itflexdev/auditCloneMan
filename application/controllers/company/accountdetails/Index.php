...
class Index extends CC_Controller
{
    ...
    public function index($pagestatus = '', $id = '')
    {
        if ($id != '') {
            ...
            if ($result) {
                ...
                redirect('company/accountdetails/index');
            }
        }
        ...
        if ($this->input->post()) {
            ...
            if ($requestData['submit'] == 'submit') {
                ...
                if ($data) {
                    $message = 'Plumber COC Invoices Type ' . (($id == '') ? 'created' : 'updated') . ' successfully.';
                }
        ...
        $data['content']          = $this->load->view('company/accountdetails/index', (isset($pagedata) ? $pagedata : ''), true);
        $this->layout2($data);
    }
    ...
    public function DTAccounts()
    {
        ...
        // print_r($post);exit();
        $totalcount = $this->Company_Model->getInvoiceList('count', ['user_id' => $userid] + $post);
        $results    = $this->Company_Model->getInvoiceList('all', ['user_id' => $userid] + $post);
